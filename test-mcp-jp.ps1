...
# 実行ファイルの存在確認
$ExePath = "DotNetFrameworkMcpServer\DotNetFrameworkMcpServer\bin\Debug\DotNetFrameworkMcpServer.exe"
if (-not (Test-Path $ExePath)) {
    Write-Host "エラー: $ExePath が見つかりません！" -ForegroundColor Red
    Write-Host "最初にプロジェクトをビルドしてください：" -ForegroundColor Yellow
    Write-Host "  dotnet build DotNetFrameworkMcpServer.sln -c Debug" -ForegroundColor Yellow
    Write-Host
